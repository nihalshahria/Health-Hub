...
exports.editInfo = catchAsync(async (req, res, next) => {
    ...
    const { phoneNo, password, specializations, qualifications, workplaces } = req.body;
    const updatedDoctor = await Doctor.findById(doctorId);
    if (!updatedDoctor) throw (new HttpError("Something went wrong, could not update information.", 500));
    let isValidPassword = false;
    isValidPassword = await bcrypt.compare(password, updatedDoctor.password);
    if (!isValidPassword) throw (new HttpError("Invalid password, could not update info.", 401));
...
exports.changePassword = catchAsync(async (req, res, next) => {
    ...
    if (!error.isEmpty()) throw (new HttpError("Invalid inputs", 422));
    const doctorId = req.params.doctorId;
    ...
    if (!isValidPassword) throw (new HttpError("Invalid password, could not update password.", 401));
    if (newPassword === oldPassword) throw (new HttpError("Old password and new password are same, could not update password.", 401));
...
exports.getDoctors = catchAsync(async (req, res, next) => {
    ...
    if (!doctors) throw (new HttpError("Could not find any doctors.", 404));
    let filteredDoctors;
